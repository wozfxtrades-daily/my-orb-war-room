...
# -------------------------------------------------
# CSS – force left menu visible + match original look
# -------------------------------------------------
st.markdown("""
<style>
...
/* Make the little arrow button very obvious */
button[kind="header"] {
...
.main .block-container {
    padding: 0.9rem 1.5rem 2rem 1.5rem !important;
...
.data-title { font-size: 10px; color: #6b7280; }
...
# -------------------------------------------------
# LEFT SIDEBAR (this is the permanent left menu)
# -------------------------------------------------
with st.sidebar:
    # Logo – click to go home
    if st.button("⬡  ORB WAR ROOM", key="home_logo", use_container_width=True):
        st.session_state.page = "War Room"
        st.rerun()

    st.markdown('<div class="logo-sub">VERSION 1.0</div>', unsafe_allow_html=True)
    ...
    # Bottom info
    ny_time = datetime.now(ZoneInfo("America/New_York")).strftime("%I:%M %p ET")
    ...
    <div class="trader">
        ...
        <div>
            ...
            <div style="font-size:11px; color:#6b7280;">NEW YORK / {ny_time}</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
